...
internal data class DockerConfig @JvmOverloads constructor(val credsStore: String? = null)

data class DockerCredential(
  @Json(name = "Username") val username: String?,
  @Json(name = "Secret") val password: String?,
)
...
object DockerCredentials {
  ...
  /**
   * If Docker is configured to use a credential store (`credsStore`), attempts to fetch a docker registry username and
   * password from the available credential store.
   *
   ...
   *
   * If no `credsStore` is defined, returns null. This SHOULD handle the case where Docker Desktop is not running, e.g.
   * in CI. If no registry is defined, defaults to https://index.docker.io/v1/.
   */
  fun getDockerCredentials(registryUrl: String?, fs: FileSystem = FileSystem.SYSTEM): DockerCredential? {
    val registry = registryUrl ?: DEFAULT_DOCKER_REGISTRY_URL

    return getDockerConfig(fs)?.credsStore?.let { fetchCredentials(it, registry) }
  }
  ...
  private fun getDockerConfig(fs: FileSystem): DockerConfig? =
    "${System.getProperty("user.home")}/.docker/config.json"
      .toPath()
      .takeIf { fs.exists(it) }
      ?.let { path -> fs.read(path) { readUtf8() } }
...
fun DefaultDockerClientConfig.Builder.withMiskDefaults() = apply { withLocalDockerCredentials() }
